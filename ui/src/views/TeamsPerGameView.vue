  ...
  <div class="cardsRow" v-if="selected != null && selected.length > 0">
    <div class="card text-dark bg-light mb-3 text-primary" v-for="team in selected" :key="team.data">
        <div class="card-body">
          ...
          <hr>
          <h6 class="card-subtitle mb-2 text-muted stat" style="padding-bottom:10px"><b>PTS -</b> {{team.points}}</h6>
          <div class="cardsRow"> 
            <p class="text-muted text-left stat"><b>FG -</b> {{team.fieldGoals}}</p>
            <p class="text-muted text-left stat"><b>FGA -</b> {{team.fieldGoalAttempts}}</p>
            <p class="text-muted text-left stat"><b>FG% -</b> {{team.fieldGoalPercentage}}</p>
          </div>
          <div class="cardsRow" style="padding-bottom:0px"> 
            <p class="text-muted text-left stat"><b>3PT -</b> {{team.threePointers}}</p>
            <p class="text-muted text-left stat"><b>3PA -</b> {{team.threePointerAttempts}}</p>
            <p class="text-muted text-left stat"><b>3P% -</b> {{team.threePointerPercentage}}</p>
          </div>
          <hr>
          <div class="cardsRow"> 
            <p class="text-muted text-left stat"><b>RBs -</b> {{team.totalRebounds}}</p>
            <p class="text-muted text-left stat"><b>O RBs -</b> {{team.offensiveRebounds}}</p>
            <p class="text-muted text-left stat"><b>D RBs -</b> {{team.defensiveRebounds}}</p>
          </div>
          <div class="cardsRow"> 
            <p class="text-muted text-left stat"><b>ASTs -</b> {{team.assists}}</p>
            <p class="text-muted text-left stat"><b>STLs -</b> {{team.steals}}</p>
            <p class="text-muted text-left stat"><b>BLKs -</b> {{team.blocks}}</p>
          </div>
        </div>
    </div>
  </div>
  <div style="padding:15px">
    <DataTable stripedRows scrollHeight="60vh" :value="teams" :paginator="true" :rows="10"
     dataKey="teamName" v-model:filters="filters" filterDisplay="menu" paginatorTemplate="CurrentPageReport FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink RowsPerPageDropdown"
     :rowsPerPageOptions="[10,20,50]" responsiveLayout="scroll" :globalFilterFields="['teamName']"
     currentPageReportTemplate="Showing {first} to {last} of {totalRecords}"
     v-model:selection="selected" selectionMode="multiple" :metaKeySelection="false"
     @rowSelect="onRowSelect" @rowUnselect="onRowUnselect">
        ...
        <Column v-for="(col, index) of selectedColumns" :field="col.field" :header="col.header" :sortable="col.sortable" :key="col.field + '_' + index"></Column>
    </DataTable>
  </div>
</template>
  ...
  methods: {
    ...
    initFilters() {
      this.filters = {
          ...
          'teamName': {operator: FilterOperator.AND, constraints: [{value: null, matchMode: FilterMatchMode.IN}]}
      }
    },
    onRowSelect(event) {
